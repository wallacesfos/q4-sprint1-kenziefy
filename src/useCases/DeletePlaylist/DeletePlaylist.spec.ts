...
describe("Create Playlist", () => {
    it('users unatorization', async () => {
        const response = await request(app).delete("/users/playlist?artist=Billie%20Eilish&song=therefore%20I%20am")
    ...
    it('user create success', async () => {
        ...
        const response = await request(app).delete("/users/playlist?artist=Billie%20Eilish&song=therefore%20I%20am").set('authorization', `bearer ${users.text.split(':')[1].replace(/["{}]/g, '')}`)
